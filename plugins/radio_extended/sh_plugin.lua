...
local radioTypes = {"handheld_radio", "walkietalkie","longrange","duplexradio","duplexwalkie","hybridradio","hybridwalkie"}
...
ix.config.Add("beepVolumeMult", 1, "Beep noise multiplier.", nil, {
...
ix.config.Add("dontGarbleLongRange", true, "Don't garble messages through long range radio", nil, {
	category = "Extended Radio"
})
...
function playSound(target,voiceprefix,sending,distance,radioType) -- Fun new function to more easily play radio send/receive sounds
	...
	volume = volume * ix.config.Get("beepVolumeMult")
	...
	if ix.config.Get("radioSounds",true) then
		if sending then 	
			...
			if radioType == "walkietalkie" or radioType == "duplexwalkie" or radioType == "hybridwalkie" then
				target:EmitSound("extendedradio/start11.wav",range,100 - 20*distance, 0.5*volume, 10)
				timer.Simple(0.05, function() target:EmitSound("extendedradio/walkiestart1.wav", range, 110, volume, 10) end)
				timer.Simple(0.7, function() target:EmitSound("extendedradio/end11.wav", range, math.random(115,120),volume, 10) end)
			-- Regular radio sounds
				...
				timer.Simple(0.05, function() target:EmitSound("extendedradio/walkiestart1.wav", range, 110, volume, 10) end)	
				timer.Simple(0.7, function() target:EmitSound(voiceprefix.."1"..".wav", range, pitchmodAlt2,volume, 10) end)
				timer.Simple(0.77, function() target:EmitSound(voiceprefix.."1"..".wav", range, pitchmodAlt2,volume, 10) end)
			end
		else
			if radioType == "walkietalkie" or radioType == "duplexwalkie" or radioType == "hybridwalkie" then
				timer.Simple(0.3,function() target:EmitSound("extendedradio/walkieend1.wav",range,110 - 20*distance, 0.3, 10) end)
				timer.Simple(0.1,function() target:EmitSound("extendedradio/end22.wav",range,110 - 20*distance, 0.2, 10) end)
			else--if radioType == "duplexradio" or radioType == "handheld_radio" then
				target:EmitSound(voiceprefix..whichnoise..".wav", range, pitchmod,0.2, 10)
				timer.Simple(0.3,function() target:EmitSound("extendedradio/walkieend1.wav",range,110 - 20*distance, 0.3, 10) end)
			end
		end
	end
...
function endChatter(listener, distance, radioType)
	...
	if ix.config.Get("radioSounds",true) then
		...
		listener:EmitSound("extendedradio/walkiestart1.wav",range,100 - 20*distFrac, volume, 10)
...
function PLUGIN:OverwriteClasses()
	...
	do
		...
		function CLASS:CanHear(speaker, listener, data)
			...
			local listenFrequencies = {}

			for _, radioType in pairs(radioTypes) do
				local radios = inventory:GetItemsByUniqueID(radioType)
				for _, radio in pairs(radios) do
					if radio:GetData("active") then
						if radioType:find("duplex") then
							if radio:GetData("listenfrequency") then
								table.insert(listenFrequencies, radio:GetData("frequency"))
							end
						else
							if radio:GetData("frequency") then
								table.insert(listenFrequencies, radio:GetData("frequency"))
							end
						end
					end
			...
			--if SERVER or LocalPlayer():IsSuperAdmin() then
			--	print("[")
			--	print(listener, speaker)
			--	PrintTable(listenFrequencies)
			--	print("spcharacter:frequency", spcharacter:GetData("frequency"))
			--	print("character:frequency", character:GetData("frequency"))
			--	print("spcharacter:channel", spcharacter:GetData("channel"))
			--	print("character:channel", character:GetData("channel"))
			--	print("]")
			--end\

			/*local shouldRun = ix.config.Get("combineChatterEnabled", false)
				and math.random(0, 100) < ix.config.Get("combineChatterChance", 40)

			if shouldRun and speaker:IsCombine() then
				local chatterReceivers  = {}
				local chatterEntities = {}

				for _, frequency in ipairs(listenFrequencies) do
					for _, ply in pairs(player.GetAll()) do
						if !isfunction(ply.GetCharacter) or !ply:GetCharacter() then continue end
						if not ply:IsCombine() then continue end
						if ply == speaker then continue end
						...
						for _, item in pairs(inventory:GetItems()) do
							if item.uniqueID != "longrange" then continue end

							if item:GetData("active") then
								if item:GetData("frequency") == frequency then
									table.insert(chatterEntities, ply)
								end
							end
						end
					end
				...
				for _, frequency in ipairs(listenFrequencies) do
					for _, ply in pairs(player.GetAll()) do
						if !isfunction(ply.GetCharacter) or !ply:GetCharacter() then continue end
						if ply:IsCombine() then continue end
						if ply == speaker then continue end

						table.insert(chatterReceivers, ply)
					end
				end

				--[[print("chatterEntities")
				PrintTable(chatterEntities)

				print("chatterReceivers")
				PrintTable(chatterReceivers)]]

				net.Start("ixMPFChatter")
				net.WriteTable(chatterEntities)
				net.WriteInt(math.random(1, 12), 32)
				net.Send(chatterReceivers)
			end*/

			if !table.HasValue(listenFrequencies, spcharacter:GetData("frequency")) then
				return false
			end
		...
		function CLASS:OnChatAdd(speaker, text, bAnonymous, data)
			...
			if (ix.config.Get("garbleRadio",true)
			and not (ix.config.Get("dontGarbleLongRange", false) and data.lrange)) then 
				text = mangleString(text, frac)
			else 
				text = text 
			end -- Garbling happens here
	...
	do
		...
		COMMAND.indicator = "chatRadioing"
		COMMAND.alias = {"r"} -- NEW
