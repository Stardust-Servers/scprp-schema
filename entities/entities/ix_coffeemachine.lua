...
--ENT.MaxStock = 4
ENT.Items = {
	{"COFFEE", "coffee", 3, 7, "coffee"}
}

function ENT:GetStock(id)
	return self:GetNetVar("stock", {})[id] or self.Items[id][4]
end
...
if (SERVER) then
	...
	function ENT:SetStock(id, amount)
		...
		stock[id] = math.Clamp(amount, 0, self.Items[id][4])
	...
	function ENT:ResetStock(id)
		...
		if (id) then
			stock[id] = self.Items[id][4]
		else
			for k, v in ipairs(self.Items) do
				stock[k] = self.Items[k][4]
			end
		end
	...
	function ENT:Use(client)
		...
		local canRestock = false
		if character:GetFaction() == FACTION_CIVILIAN then canRestock = true end

		if (!canRestock or !client:KeyDown(IN_WALK)) then
			...
			if (self:GetStock(buttonID) > 0) then
				...
				ix.item.Spawn(itemInfo[2], self:GetPos() + self:GetForward() * forward + self:GetRight() * 4 + self:GetUp() * -26, function(item, entity)
					self:EmitSound("buttons/button4.wav", 60)

					character:TakeMoney(price)
					client:NotifyLocalized("vendingPurchased", self.Items[buttonID][5], ix.currency.Get(price))
		...
		elseif (client:KeyDown(IN_WALK) and self:GetStock(buttonID) == 0) then
			self:ResetStock(buttonID)

			client:NotifyLocalized("vendingRestocked", self.Items[buttonID][5])
			self.nextUsetime = CurTime() + 3
		end
	end
	...
	function ENT:Draw()
		...
		cam.Start3D2D(position + forward * 17.36 + right * -19.2 + up * 6, angles, 0.06)
